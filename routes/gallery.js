...
const { getGallery, saveArtwork, deleteArtwork, getArtworkDetails, toggleFavorite } = require('../services/galleryService');
...
router.patch('/:artworkId/favorite', async (req, res) => {
  try {
    const { artworkId } = req.params;
    
    const artwork = await toggleFavorite(artworkId);
